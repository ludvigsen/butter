 ...
 */
module.exports = function( Project, Profile ) {
  ...
  return {
    ...
    find: function( options, callback ) {
      ...
      getProjectsWhere( where, function( err, results ) {
      	...
      	var thisProject=results[0];
      	if (thisProject.remixedFrom != null) {
      		var remixFindOptions={id:thisProject.remixedFrom};
      		getProjectsWhere( remixFindOptions, function( err, remixResults ) {
      			var remixedProject=remixResults[0];
      			thisProject.remixedFromName=remixedProject.name;
      			var remixedFromAuthorName="";
      			var remixedFromUserID="";
      			Profile.find({
					where: ['email=?',remixedProject.email]
				}).complete(function(err,theProfile) {
					if (theProfile.displayName != null && theProfile.displayName != "") {
						remixedFromAuthorName += theProfile.displayName + " ";
					}
					if (theProfile.twitterHandle != null && theProfile.twitterHandle != "") {
						//remixedFromAuthorName += theProfile.twitterHandle; 
					}
					if (theProfile.id != null & theProfile.id != "") {
						remixedFromUserID=theProfile.id;
					}
					thisProject.remixedFromAuthorName=remixedFromAuthorName;
					thisProject.remixedFromUserID=remixedFromUserID;
					//thisProject.remixedFromUserID=theProfile.id;
					callback(err,thisProject); 
				});
    ...
    update: function updateProject( options, callback ) {
      ...
      .success(function( project ) {
        ...
        //note that we don't update the featured attribute - only done from our showcase admin interface
